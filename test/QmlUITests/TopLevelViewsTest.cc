...
void TopLevelViewsTest::_testNavigateViews()
{
    ...
    // Navigate through settings pages that are unconditionally visible
    {
        ...
        // The sidebar button is named after the page's display name, while the page's
        // objectName comes from SettingsPages.json "id" when set (see
        // tools/generators/settings_qml/README.md). They differ for localized displays.
        struct SettingsPageEntry
        {
            QString displayName;
            QString objectNameSuffix;
        };

        const QList<SettingsPageEntry> settingsPages = {
            {QStringLiteral("General"),        QStringLiteral("General")},
            {QStringLiteral("Fly View"),       QStringLiteral("FlyView")},
            {QStringLiteral("Plan View"),      QStringLiteral("PlanView")},
            {QStringLiteral("ADSB Server"),    QStringLiteral("ADSBServer")},
            {QStringLiteral("Comm Links"),     QStringLiteral("CommLinks")},
            {QStringLiteral("App Logging"),    QStringLiteral("AppLogging")},
            {QStringLiteral("App Log Viewer"), QStringLiteral("AppLogViewer")},
            {QStringLiteral("地图"),           QStringLiteral("Maps")},
            {QStringLiteral("Remote ID"),      QStringLiteral("RemoteID")},
        ...
        for (const SettingsPageEntry &entry : settingsPages) {
            const QString buttonName = QStringLiteral("settingsButton_") + entry.displayName;
            ...
            const QString expectedObjectName = QStringLiteral("settingsPage_") + entry.objectNameSuffix;
