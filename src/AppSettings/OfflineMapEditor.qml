...
FlightMap {
    ...
    // Map names are never translated in C++ (they identify the tiles in the cache), so the
    // untranslated name is translated here for display only.
    function _mapTypeDisplay(mapTypeName) {
        if (mapTypeName === "Various") {
            return qsTr("Various")
        }
        return qsTranslate("QObject", mapTypeName)
    }
                    ...
                    text: {
                        if (tileSet) {
                            if (tileSet.defaultSet)
                                ...
                                return "(" + _mapTypeDisplay(tileSet.mapTypeStr) + ")"
                        } else
                            return ""
                    }
                    horizontalAlignment: Text.AlignHCenter
                    ...
                    Column {
                        ...
                        Column {
                            ...
                            QGCComboBox {
                                ...
                                anchors.left: parent.left
                                anchors.right: parent.right
                                model: QGroundControl.mapEngineManager.mapList.map(function (name) { return _mapTypeDisplay(name) })
                                onActivated: (index) => {
                                    mapType = QGroundControl.mapEngineManager.mapList[index]
                                }
                                Component.onCompleted: {
                                    var index = QGroundControl.mapEngineManager.mapList.indexOf(mapType)
                                    if (index === -1) {
