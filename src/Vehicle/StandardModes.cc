...
/// Mode names arriving in AVAILABLE_MODES are untranslated source strings. Resolve them through the
/// firmware plugin's translation context so the plugin's own translations are reused, then fall back
/// to the standard mode names authored by QGC.
static QString _localizedModeName(Vehicle *vehicle, const QString &modeName)
{
    const QByteArray source = modeName.toUtf8();
    const QString pluginTranslation = QCoreApplication::translate(vehicle->firmwarePlugin()->metaObject()->className(), source.constData());
    if (pluginTranslation != modeName) {
        return pluginTranslation;
    }
    return QCoreApplication::translate("StandardModes", source.constData());
}
...
void StandardModes::gotMessage(MAV_RESULT result, const mavlink_message_t &message)
{
    ...
    if (result == MAV_RESULT_ACCEPTED) {
        ...
        name = _localizedModeName(_vehicle, name);
