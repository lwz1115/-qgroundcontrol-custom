...
void RemoteIDManager::_sendSystem()
{
    ...
    if (locationType == LocationTypes::FIXED) {
        ...
        if (lat >= -90.0 && lat <= 90.0 && lon >= -180.0 && lon <= 180.0) {
            ...
            _updateGcsPositionStatus(false, tr("The provided coordinates for FIXED position are invalid."));
        }
    } else {
        ...
        if (!geoPositionInfo.isValid()) {
            ...
            _updateGcsPositionStatus(false, tr("GCS GPS data error: %1").arg(positionManager->gcsPositioningError()));
        } else if (!gcsPosition.isValid() || gcsPosition.type() == QGeoCoordinate::InvalidCoordinate) {
            _updateGcsPositionStatus(false, tr("GCS GPS data error: Invalid coordinate type."));
            ...
            _updateGcsPositionStatus(false, tr("GCS GPS data error: Altitude data is mandatory for FAA regions."));
        } else if (!gcsPositionTimestamp.isValid() || (gcsPositionTimestamp.msecsTo(QDateTime::currentDateTimeUtc()) > ALLOWED_GPS_DELAY)) {
            _updateGcsPositionStatus(false, tr("GCS GPS data is older than 5 seconds"));
        } else {
            _updateGcsPositionStatus(true);
        }
    }
