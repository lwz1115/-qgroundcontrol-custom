...
PositionResult getVehicleGPSPosition()
{
    ...
    if (isSaneCoord(lat, lon)) {
        const QString label = QCoreApplication::translate("NTRIPGgaProvider", "Vehicle GPS");
        return {QGeoCoordinate(lat, lon, veh->coordinate().altitude()), label};
    }
    return {};
}
...
PositionResult getVehicleEKFPosition()
{
    ...
    if (coord.isValid() && isSaneCoord(coord.latitude(), coord.longitude())) {
        const QString label = QCoreApplication::translate("NTRIPGgaProvider", "Vehicle EKF");
        return {coord, label};
    }
    return {};
}
...
PositionResult getRTKBasePosition()
{
    ...
    if (isSaneCoord(lat, lon)) {
        const QString label = QCoreApplication::translate("NTRIPGgaProvider", "RTK Base");
        return {QGeoCoordinate(lat, lon, alt), label};
    }
    return {};
}
...
PositionResult getGCSPosition()
{
    ...
    if (coord.isValid() && isSaneCoord(coord.latitude(), coord.longitude())) {
        const QString label = QCoreApplication::translate("NTRIPGgaProvider", "GCS Position");
        return {coord, label};
    }
    return {};
}
