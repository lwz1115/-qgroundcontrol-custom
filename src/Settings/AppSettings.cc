...
DECLARE_SETTINGSFACT(AppSettings, samplePointMinSpacing)
DECLARE_SETTINGSFACT(AppSettings, missionLoopCount)
DECLARE_SETTINGSFACT(AppSettings, missionReturnHomeAfterLoop)
...
DECLARE_SETTINGSFACT_NO_FUNC(AppSettings, qLocaleLanguage)
{
    if (!_qLocaleLanguageFact) {
        ...
        // An unsupported stored value falls back to the built-in default language
        if (_qLocaleLanguageFact->enumIndex() == -1) {
            _qLocaleLanguageFact->setRawValue(QLocale::Chinese);
        }
    }
...
/// An unset or unsupported value falls back to Chinese, which is the shipped default UI language.
