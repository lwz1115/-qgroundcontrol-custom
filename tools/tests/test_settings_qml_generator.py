...
class TestGeneratePageQml:
    ...
    def test_heading_id_provides_object_name_for_non_ascii_heading(self, settings_dir: Path):
        # A heading with no ASCII characters cannot produce an identifier on its own, so
        # headingId supplies one while the visible heading text stays untouched.
        page = PageDef(
            groups=[
                GroupDef(
                    heading="访问令牌",
        ...
        qml = generate_page_qml(page, settings_dir)
        assert 'objectName: "settingsGroup_Tokens"' in qml
        assert 'heading: qsTr("访问令牌")' in qml

    def test_heading_id_must_be_identifier_safe(self, tmp_path: Path):
        data = {
            "version": 1,
            "groups": [
                {
                    "heading": "访问令牌",
                    "headingId": "令牌",
                    "controls": [{"setting": "appSettings.savePath"}],
                },
            ],
        }
        with pytest.raises(ValueError, match=r"only \[A-Za-z0-9_\]"):
            load_page_def(_make_page_json(tmp_path, data))
...
class TestRealPageDefinitions:
    ...
    def test_localized_maps_page_uses_ascii_object_names(self, repo_root: Path):
        # The Maps page shows localized text but must keep ASCII objectNames so
        # test/QmlUITests/TopLevelViewsTest.cc can still look the page up.
        pages_dir = repo_root / "src" / "AppSettings" / "pages"
        settings_dir = repo_root / "src" / "Settings"
        page = load_page_def(pages_dir / "Maps.SettingsUI.json")
        qml = generate_page_qml(
            page, settings_dir, json_context="Maps.SettingsUI.json", page_name="Maps"
        )
        assert 'objectName: "settingsPage_Maps"' in qml
        assert 'objectName: "settingsGroup_Tokens"' in qml
        assert 'objectName: "settingsGroup_MapboxLogin"' in qml
        assert 'objectName: "settingsGroup_CustomMapURL"' in qml
        assert 'objectName: "settingsGroup_TileCache"' in qml
