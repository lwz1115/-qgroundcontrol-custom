...
def main() -> int:
    ...
    for entry in pages_data.get("pages", []):
        ...
        # 'name' is the display text (any language); 'id' is the optional ASCII identifier
        # used for the page's objectName so UI tests keep a grep-able hook.
        page_name = entry.get("id") or entry.get("name", "")
        qml = generate_page_qml(
            page, SETTINGS_DIR, json_context=page_def_name, page_name=page_name
        )
