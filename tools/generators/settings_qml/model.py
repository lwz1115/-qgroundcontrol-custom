...
class GroupDef:
    ...
    @property
    def object_name_source(self) -> str:
        """Text the group's objectName is derived from.

        A non-ASCII heading (e.g. a translated one) cannot produce an identifier on
        its own, so authors supply an explicit ``headingId`` instead."""
        return self.headingId or self.heading
...
_ALLOWED_GROUP_KEYS = frozenset({
    "comment", "heading", "headingId", "showWhen", "enableWhen", "headingDescription",
...
def load_page_def(json_path: Path) -> PageDef:
    ...
    for grp_data in require_list(data.get("groups", []), "'groups'", json_path):
        ...
        grp = GroupDef(
            ...
            headingId=require_object_name_id(
                grp_data.get("headingId", ""), "group headingId", json_path
            ),
