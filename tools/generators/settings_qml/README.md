...
| `id` | string | ASCII identifier (`[A-Za-z0-9_]+`) for the page's objectName; required when `name` has no ASCII characters |
...
| `headingId` | string | no | ASCII identifier (`[A-Za-z0-9_]+`) for the group's objectName; required when `heading` has no ASCII characters |
...
| Item | objectName |
| --- | --- |
| Page root | `settingsPage_<id or PageName>` (characters outside `[A-Za-z0-9_]` stripped, e.g. `settingsPage_RemoteID`) |
| Group (`SettingsGroupLayout`, headed groups only) | `settingsGroup_<headingId or Heading>` (characters outside `[A-Za-z0-9_]` stripped, e.g. `settingsGroup_EUVehicleInfo`) |
...
A heading written in a non-ASCII script (or one that translates to one) cannot
produce an identifier at all, so such a group sets `headingId` to an explicit ASCII
identifier. The same applies to a page whose display `name` is non-ASCII: give the
entry in `SettingsPages.json` an `id`. Both are display-free — they exist only for
the objectName, so they can stay English while the visible text is localized:

```json
{ "heading": "访问令牌", "headingId": "Tokens", "controls": [ ] }

{ "name": "地图", "id": "Maps", "qml": "MapsSettings.qml", "pageDefinition": "Maps.SettingsUI.json" }
```
