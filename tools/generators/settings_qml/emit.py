...
def generate_page_qml(
    ...
    for grp_idx, grp in enumerate(page.groups):
        ...
        if grp.heading:
            sanitized = _object_name(grp.object_name_source)
            if not sanitized:
                raise ValueError(
                    f"Page '{page_name or json_context}': heading '{grp.heading}' sanitizes to an "
                    f"empty objectName. Use a heading with ASCII letters or digits, or add a "
                    f"'headingId' with an ASCII identifier."
                )
...
_ALLOWED_PAGE_ENTRY_KEYS = frozenset({
    "comment", "divider", "name", "id", "url", "qml", "icon", "visible", "pageDefinition",
})
