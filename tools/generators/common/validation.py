...
_OBJECT_NAME_ID_RE = re.compile(r"[A-Za-z0-9_]+")


def require_object_name_id(value: object, context: str, source: object) -> str:
    """Validate an optional hand-written identifier used inside a QML objectName.

    Empty means "not supplied". Anything else must already consist of identifier
    characters: a value that has to be sanitized would silently drop characters and
    could collide with another group's objectName.
    """
    identifier = require_qml_safe_string(value, context, source)
    if identifier and not _OBJECT_NAME_ID_RE.fullmatch(identifier):
        raise ValueError(
            f"{source}: {context} must contain only [A-Za-z0-9_], got: {clamped_repr(identifier)}"
        )
    return identifier
